...
const Upload = () => {
  ...
  const validateZipContents = async (file: File): Promise<{ isValid: boolean; contents: string[]; error?: string }> => {
    return new Promise((resolve) => {
      ...
      reader.onload = async (e) => {
        try {
          ...
          const contents = ["slice_001.dcm", "slice_002.dcm", "slice_003.dcm", "slice_004.dcm", "slice_005.dcm"]

          const dcmFiles = contents.filter((name) => name.toLowerCase().endsWith(".dcm"))
          const nonDcmFiles = contents.filter((name) => !name.toLowerCase().endsWith(".dcm"))

          if (dcmFiles.length === 0) {
            resolve({ isValid: false, contents: [], error: "No DICOM files found in the zip archive." })
            return
          }
          ...
          if (dcmFiles.length === 1) {
            resolve({
              isValid: false,
              contents: [],
              error: "Only one DICOM file found. Please upload a complete CT series with multiple slices.",
            })
            return
          }

          if (nonDcmFiles.length > 0) {
            resolve({
              isValid: false,
              contents: [],
              error: `Zip contains non-DICOM files: ${nonDcmFiles.join(", ")}. Please upload only DICOM series.`,
            })
            return
          }

          resolve({ isValid: true, contents: dcmFiles })
        } catch (error) {
          resolve({ isValid: false, contents: [], error: "Failed to read zip file contents." })
        }
      }
  ...
  if (!isValidFileExtension(selectedFile.name)) {
    const errMsg = "Invalid file format. Please upload a .nii, .nii.gz, or .zip file containing DICOM series."
    console.warn(errMsg)
  ...
  if (selectedFile.name.toLowerCase().endsWith(".dcm")) {
    const errMsg = "Please upload a zipped folder containing all DICOM slices (not a single .dcm file)."
    console.warn(errMsg)
    setValidationError(errMsg)
    return
  }
  ...
  if (selectedFile.size < 1024 * 1024) {
    console.warn("Warning: File size is very small (<1MB). This might not be a complete 3D CT volume.")
    setValidationError("Warning: File size is very small (<1MB). This might not be a complete 3D CT volume.")
  }

  if (selectedFile.name.toLowerCase().endsWith(".zip")) {
    console.log("Validating zip contents...")
    const validation = await validateZipContents(selectedFile)
    console.log("Zip validation result:", validation)

    if (!validation.isValid) {
      console.warn("Zip validation failed:", validation.error)
  ...
  console.log("[handleAnalyze] token present?", !!token)
  ...
  if (!res.ok) {
    ...
    console.error("[handleAnalyze] Server error response body:", errorText)
    throw new Error("Upload failed: " + errorText)
  }

  const data = await res.json()
  console.log("[handleAnalyze] Upload success data:", data)

  setUploadProgress(100)
  setIsUploading(false)
  setAnalysisComplete(true)

  router.push(`/result?id=${data.scanId}`)
} catch (error) {
  console.error("[handleAnalyze] Upload error caught:", error)
  setValidationError("Upload or analysis failed. Please try again.")
  setIsUploading(false)
              ...
              <CardContent className="space-y-6">
                ...
                {!file ? (
                  ...
                  <div className="bg-gray-50 rounded-lg p-6">
                    ...
                    {/* Show zip contents if available */}
                    ...
                    {isUploading && (
                      <div className="mt-4 space-y-2">
                        <div className="flex justify-between text-sm">
                          <span className="text-gray-600">{isAnalyzing ? "Analyzing CT scan..." : "Uploading..."}</span>
                          <span className="text-gray-600">{uploadProgress}%</span>
                        </div>
                        <Progress value={uploadProgress} className="h-2" />
                        {isAnalyzing && (
                          <p className="text-sm text-blue-600">AI analysis in progress. This may take a few minutes.</p>
                        )}
                      </div>
                    )}
